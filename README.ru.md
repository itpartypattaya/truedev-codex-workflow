...
Установка остаётся на той версии, с которой её сделали, пока кто-нибудь не обновит, — и устаревшая
установка никак себя не проявляет, если о ней не сказать. В начале сессии плагин сравнивает
установленную версию с клоном маркетплейса, который уже лежит у вас на диске (чтение файла, без
запросов), и печатает одну строку, если там есть версия новее:

```text
TrueDev Workflow 1.1.16 is available; 1.1.13 is installed.
```

Сам он ничего не обновляет. Обновление плагина запускает у вас новый код, и это ваше решение,
а не решение хука.

| Переменная окружения | Что делает |
| --- | --- |
| не задана (по умолчанию) | Только локальное сравнение, не чаще раза в сутки |
| `TRUEDEV_UPDATE_CHECK=network` | Дополнительно тянет опубликованный манифест по HTTPS из этого репозитория |
| `TRUEDEV_UPDATE_CHECK=off` | Не проверять вовсе |

Сетевая половина включается вручную, потому что «сходить в сеть» — это другое обещание, чем
«прочитать файл». Запрос не несёт ни идентификаторов, ни сведений о репозитории: он спрашивает у
публичного файла, какая там версия. Ошибка проглатывается молча — уведомление о версии не должно
уметь сломать сессию. Подробности в `PRIVACY.md`.

### Четыре подключения хуков

| Событие | Что делает |
| --- | --- |
| `SessionStart(compact)` | Снимает контрольную точку сжатия и вставляет короткую сводку состояния из разрешённого списка полей |
| `SessionStart(startup\|resume)` | Сообщает об устаревшей установке. Ничего не блокирует — см. [Обновления](#обновления) |
| `PreToolUse(Bash\|apply_patch\|Edit\|Write\|Agent\|spawn_agent\|mcp__.*)` | Отклоняет подходящие вызовы инструментов при открытом гейте, кроме собственных read-only команд раннера |
| `Stop` | Проверяет состояние в конце хода, чтобы процесс не бросил шаг на полпути |

### Чего хуки не гарантируют

Об этом стоит говорить точно, потому что весь смысл — «хуки надёжнее уговоров»:

- **Состояние пишет раннер, а не модель.** Это главное отличие от workflow на одних промптах:
  переход обязан пройти проверку до записи, поэтому шаг не может тихо понизить собственный гейт.
  Что модель всё ещё может — написать в тексте, что проверка выполнялась, когда она не
  выполнялась. Состояние фиксирует переход, а не правду о работе за ним.
- **Pre-tool-хук видит только то, о чём сообщает хост.** Хостинговые и специализированные
  инструменты могут вообще не присылать событий, а шаблон, покрывающий все инструменты, стоил бы
  около полусекунды на каждый вызов. Что попало под шаблон — блокируется при открытом гейте; что
  не попало — на то и не претендовали.
- **`Bash` под шаблоном, но read-only команды раннера проходят.** Этот список и позволяет
  собирать доказательства при открытом гейте. Он сверяет команду целиком, поэтому аргументом
  вторую команду не протащить.
- **Всё ниже слоя хуков — обычный текст.** Что пишет TEST и как готовит изменения REVIEW —
  инструкции модели. Хуки ограничивают последовательность, а не содержание.
- **Любой, у кого есть право записи, может это выключить.** Удаление `.truedev-workflow/`
  прекращает контроль. Это задуманный выход, а не дыра. Повреждённое состояние, наоборот,
  блокирует работу; когда состояния нет вовсе, плагин не делает ничего.

Настоящая граница — права в репозитории, песочница, защищённые ветки, CI и авторизация на
стороне провайдера. Это помогает длинной задаче пережить длинную сессию, но не является средством
безопасности.

### Сколько это стоит в контексте

Сессия всегда несёт два описания скиллов — именно по ним модель решает, применим ли скилл:
**около 1000 символов, примерно 255 токенов**. Хуки не стоят контекста вообще: они выполняются в
обвязке, а не в модели. Вызов скилла подгружает его инструкции — вот там основная стоимость, и
только когда вы им реально пользуетесь.
